...
import com.example.dirvingpatner.databinding.ActivityPanCardUploadBinding

class PanCardUpload : AppCompatActivity() {

    private lateinit var binding: ActivityPanCardUploadBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityPanCardUploadBinding.inflate(layoutInflater)
        ...
        binding.continueBtn.setOnClickListener{
            ...
            sharedPref.putBoolean("isPanCardComplete", true)
            sharedPref.apply() // Apply the changes

            // Start the RCUpload activity when the "Continue" button is clicked
            val intent = Intent(this, RcUpload::class.java)
            startActivity(intent)
        }
    }

    // Function to customize the status bar appearance ( Blue Color to match toolbar Color)
